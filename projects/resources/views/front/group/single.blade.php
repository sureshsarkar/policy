                    ...
                    <div class="blog-detail-left">
                        <div class="blog-detail-image">
                            @if ($data->featureImage)
                                <img src="{{ asset($data->featureImage) }}" alt="{{ $data->title }}"
                                    title="{{ $data->title }}">
                            @endif
                        </div>
                        ...
                        <ul>
                            @foreach (App\Models\Blogs\BlogCategory::orderBy('id', 'desc')->get() as $c)
                                <li class="cat-item cat-item-2"><a
                                        href="{{ url($c->seo_url) }}/">{{ Helper::getLangName($c, 'title') }}</a>
                                    <span>({{ App\Models\Blogs\Blog::where('blog_category_id', $c->id)->count() }})</span>
                                </li>
                            @endforeach
                        </ul>
                    </section>

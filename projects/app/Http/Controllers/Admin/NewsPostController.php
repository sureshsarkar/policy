...
class NewsPostController extends Controller{
    ...
    public function __construct(NewsPost $model){
        $this->model=$model;
        $this->admin_base_url="news-post.index";
        $this->admin_view="admin.news-post";
    }

    
    public function index(){
        $data=$this->model::with('parentname')->orderBy("id","desc")->get();
        // return $data;
    ...
    public function store(Request $request){
        $validator = Validator::make($request->all(), [
            'seo_url' => 'required|unique:news-post,seo_url',
            'name' => 'required'
        ]);   
        if($validator->fails()){
        ...
        if ($request->hasFile("image")) {
            $data['image'] = Upload::fileUpload($request->file("image"),"news-post");
        }
        if ($request->hasFile("bannerImage")) {
            $data['bannerImage'] = Upload::fileUpload($request->file("bannerImage"),"news-post");
        }
    ...
    public function update(Request $request, $id){
        $validator = Validator::make($request->all(), [
            'seo_url' => 'required|unique:news_post,seo_url,'.$id,
            'name' => 'required'
        ]);   
        ...
        if($exist){
            ...
            if ($request->hasFile("image2")) {
                $data['image2'] = Upload::fileUpload($request->file("image2"),"news-post");
